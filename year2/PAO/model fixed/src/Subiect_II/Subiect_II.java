...
//        List<Automobil> ln = la.stream().filter(a -> a.getCapacitate() >= 2000 && a.getCapacitate() <= 3000).collect(toList());
//        ln.stream().forEach(System.out::println);
//        System.out.println();

//        System.out.println("Cerinta d)");
//        List<Automobil> ln = la.stream().filter(a -> a.getMarca().equals("Audi")).max(comparing(Automobil::getPret)).stream().collect(toList());
//        System.out.println(ln.get(0).getPret());
        //System.out.println(la.stream().filter(a -> a.getMarca().equals("Audi")).max(comparing(Automobil::getPret)).getClass());

//        HashMap<String, List<Automobil>> H = new HashMap<>();
//
//        List<String> listaMarci = la.stream().map(Automobil::getMarca).distinct().collect(toList());
//        System.out.println(listaMarci);
//        for ( String s : listaMarci){
//            H.put(s, la.stream().filter(ob -> ob.getMarca().equals(s)).collect(toList()));
//        }
//        System.out.println(H);

      //  System.out.println(la.stream().map(Automobil::getMarca).sorted().collect(toList()));

            la.stream().filter(ob -> ob.getMarca().equals("BMW")).filter(ob -> ob.getPret() <= 6000).sorted(comparing(Automobil::getPret)).forEach(System.out::println);
           // la.stream().map(Automobil::getMarca).distinct().forEach(System.out::println);
//        List<Automobil> list = la.stream().filter(ob -> ob.getMarca().equals("Audi")).filter(ob -> ob.getPret() >= 2000 && ob.getPret() <= 3000).collect(toList());
//        System.out.println(list);
            la.stream().map(Automobil::getMarca).distinct().forEach(marca -> System.out.println(la.stream().filter(ob -> ob.getMarca().equals(marca)).collect(toList())));
