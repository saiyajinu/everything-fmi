...
def citire():
    ...
    if (str(s.rstrip("\n")) == "States:"):      #verifica daca in fisier suntem la setul de stari apoi adauga datele la lista corespunzatoare
                                                #starea de accept se trece ca o noua ultima stare
        s = f.readline()
        while str(s.rstrip("\n")) != "End":
            s = s.strip()
            state_line = s.split(" ,")
...
#Exemplul folosit: validarea w#w ,w={0,1} (Exercitiul 4)
#Parcurgerea
...
if start==stop:
    #Cuvantul este acceptat, starea curenta este starea finala, dar intra in urmatoarea stare de verificarea tapeurilor
    #Exercitiul 3
    accept = ""
    headpos = 1
    while accept == "":
...
print("Tape1: ",*tape1[1:len(tape1)-1])
print("Tape2: ",*tape2[1:len(tape2)-1])
