...
public:
    ...
    friend ostream& operator<<(ostream& out, const Produs& prod){
        ...
        out<<"\nProdusul are "<<prod.nrRatinguri<<" rating(uri)";
        if (prod.nrRatinguri)
            out<<"\nRating(urile): ";
        for (int i=0; i<prod.nrRatinguri; i++)
            out<<prod.rating[i]<<"/10 ";
        out<<"\nCuloarea produsului este "<<prod.culoare<<endl<<endl;
        cout<<endl;
        return out;
    }
    ...
    void produsEsteNegru(){
        if (this->culoare[0] == 'n')
            cout<<this->nume<<" are culoarea neagra"<<endl<<endl;
        else cout<<this->nume<<" nu are culoarea neagra"<<endl<<endl;
    }

    void afisareRatinguriProdus(){
        if (this->nrRatinguri){
            cout<<"Rating(urile) produsului "<<this->nume<<": ";
            for(int i = 0; i < nrRatinguri; i++)
                cout<<rating[i]<<"/10 ";}
        else cout<<this->nume<<" nu are ratinguri";
        cout<<endl<<endl;
    }
};
